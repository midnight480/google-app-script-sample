...
clasp (Command Line Apps Script Projects) を使用してローカル環境で開発します。
詳細なインストール手順やチュートリアルについては以下を参照してください：
- [clasp を使用した Apps Script の開発（公式ガイド）](https://developers.google.com/apps-script/guides/clasp?hl=ja)
- [clasp Codelab（チュートリアル）](https://codelabs.developers.google.com/codelabs/clasp?hl=ja)

**事前準備（重要）:**
clasp を使用するには、[Google Apps Script のユーザー設定](https://script.google.com/home/usersettings) にアクセスし、「Google Apps Script API」をオンにする必要があります。

```bash
# Node.js がインストールされていることを確認後、claspのインストール
npm install -g @google/clasp

# Google Apps Scriptにログイン (ブラウザが開いて認証を行います)
clasp login

# プロジェクトの作成（各スクリプトのディレクトリ内で実行）
